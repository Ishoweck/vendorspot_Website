...
function RecommendedContent() {
  ...
  const { data: products, total, totalPages, loading } = usePagedApi<Product>(
    `/products/recommended?limit=${PAGE_SIZE}&page=${page}`
  );
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-3 mb-8 pt-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-2xl sm:text-3xl font-bold text-primary">Recommended for You</h1>
                <span className="text-[10px] font-bold uppercase tracking-wider bg-primary/10 text-primary px-2.5 py-1 rounded-full">Personalised</span>
              </div>
              {!loading && total > 0 && (
          ...
          {!loading && (!products || products.length === 0) && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <p className="text-lg font-bold text-dark mb-2">Nothing to recommend yet</p>
              <p className="text-sm text-gray-400 mb-6">Browse more products so we can personalise your feed.</p>
...
export default function RecommendedPage() {
  return <Suspense><RecommendedContent /></Suspense>;
}
