  ...
  return (
    <section className="bg-dark text-center px-6 pt-20 sm:pt-28 pb-16 sm:pb-20">
      ...
      <motion.p
        ...
        className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-accent mb-4"
      >
      ...
      >
        {appButtons.map(({ label, href, path }) => (
          <motion.a
            ...
            whileTap={{ scale: 0.97 }}
            className="flex items-center gap-2.5 border border-white/25 text-white rounded-xl px-5 py-3 text-sm font-medium transition-colors"
          >
            <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current shrink-0"><path d={path} /></svg>
            {label}
          </motion.a>
        ))}
      </motion.div>
